...
export class FriendsRule extends BaseRule {
  ...
  /**
   * 🔄 ОБНОВЛЕНО: Можно ли применить +n ЧЕРЕЗ ДРУЗЕЙ?
   *
   * НОВАЯ ЛОГИКА (согласно физике абакуса):
   * - Проверяем физическую возможность применить формулу +n = +10 - friend
   * - НЕ требуем чтобы прямое действие было невозможно
   * - Правило Друзья может использоваться всегда когда физически возможно
   *
   * @param {number} v - текущее значение разряда (0-9)
   * @param {number} n - сколько добавить (1-9)
   * @param {number[]} fullState - состояние всех разрядов
  ...
  canApplyPlusFriend(v, n, fullState, position) {
    // Используем новый метод физической проверки
    return this.canApplyFriendAddition(n, v, fullState, position);
  }
  ...
  canApplyMinusFriend(v, n, fullState, position) {
    // Используем новый метод физической проверки
    return this.canApplyFriendSubtraction(n, v, fullState, position);
  }
  ...
  /**
   * 🆕 Проверяет можно ли физически применить действие +digit по правилу Друзья
   *
   * Для каждой цифры есть требования к состоянию ТЕКУЩЕГО разряда:
   * +1: нужно 9 (все бусины активны)
   * +2: нужно 8 или 9 (верхняя + 3-4 нижние)
   * +3: нужно 7-9 (верхняя + 2-4 нижние)
   * +4: нужно 6-9 (верхняя + 1-4 нижние)
   * +5: нужно 5-9 (верхняя активна)
   * +6: нужно 4-9 (минимум 4 активны)
   * +7: нужно 3-9 (минимум 3 активны)
   * +8: нужно 2-9 (минимум 2 активны)
   * +9: нужно 1-9 (минимум 1 активна)
   *
   * @param {number} digit - цифра 1-9
   * @param {number} currentValue - текущее значение разряда (0-9)
   * @param {number[]} fullState - полное состояние всех разрядов
   * @param {number} position - позиция текущего разряда
   * @returns {boolean}
   */
  canApplyFriendAddition(digit, currentValue, fullState, position) {
    if (digit < 1 || digit > 9) return false;

    // Формула: +digit = +10 - friend, где friend = 10 - digit
    const friend = 10 - digit;

    // Требование 1: Можно ли добавить +10 к следующему разряду?
    if (!this._canAddTen(fullState, position)) {
      return false;
    }

    // Требование 2: Текущий разряд должен позволять вычесть friend
    // Минимальное значение для применения формулы
    const minRequired = friend;

    if (currentValue < minRequired) {
      return false; // Недостаточно активных бусин для вычитания friend
    }
   ...
   * 🆕 Проверяет можно ли физически применить действие -digit по правилу Друзья
   *
   * Для каждой цифры есть требования:
   * -1: текущий разряд 0, следующий 1-4
   * -2: текущий разряд 0-1, следующий 1-4
   * -3: текущий разряд 0-2, следующий 1-4
   * -4: текущий разряд 0-3, следующий 1-4
   * -5: текущий разряд 0-4, следующий 1-4
   * -6: текущий разряд 0-3, следующий 1-4
   * -7: текущий разряд 0-2, следующий 1-4
   * -8: текущий разряд 0-1, следующий 1-4
   * -9: текущий разряд 0, следующий 1-4
   *
   * @param {number} digit - цифра 1-9
   * @param {number} currentValue - текущее значение разряда (0-9)
   * @param {number[]} fullState - полное состояние всех разрядов
   ...
   * @returns {boolean}
   */
  canApplyFriendSubtraction(digit, currentValue, fullState, position) {
    if (digit < 1 || digit > 9) return false;

    // Формула: -digit = -10 + friend, где friend = 10 - digit
    const friend = 10 - digit;

    // Требование 1: Можно ли убрать -10 из следующего разряда?
    if (!this._canSubtractTen(fullState, position)) {
      return false;
    }

    // Требование 2: Текущий разряд должен позволять добавить friend
    // Максимальное значение которое может быть в разряде ДО добавления friend
    const maxAllowed = 9 - friend;

    if (currentValue > maxAllowed) {
      return false; // Слишком много активных бусин, friend не поместится
    }
  ...
  /**
   * 🆕 Получить минимальное состояние разряда для применения +digit
   *
   * @param {number} digit - цифра 1-9
   * @returns {number} минимальное значение разряда (например, для +1 это 9)
   */
  getMinStateForFriendAddition(digit) {
    if (digit < 1 || digit > 9) return 9;
    const friend = 10 - digit;
    return friend; // Минимум friend активных бусин
  }

  /**
   * 🆕 Получить максимальное состояние разряда для применения -digit
   *
   * @param {number} digit - цифра 1-9
   * @returns {number} максимальное значение разряда (например, для -1 это 0)
   */
  getMaxStateForFriendSubtraction(digit) {
    if (digit < 1 || digit > 9) return 0;
    const friend = 10 - digit;
    return 9 - friend; // Максимум (9 - friend) активных бусин
  }
