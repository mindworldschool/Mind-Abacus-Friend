...
export function generateExample(settings = {}) {
  try {
    ...
    if (friendsActive === true) {
      ...
      // ⚠️ КРИТИЧНО: Правило Друзья ТРЕБУЕТ минимум 2 разряда!
      // Потому что формула +n = +10 - friend требует перехода на следующий разряд
      if (digitCount < 2) {
        const errorMsg = `❌ Правило Друзья НЕ МОЖЕТ работать с ${digitCount} разрядом!
Формула Друзья требует переход на следующий разряд:
  +1 = +10 (в следующем разряде) - 9
  +2 = +10 (в следующем разряде) - 8
  и т.д.

Для работы правила Друзья установите разрядность минимум 2.`;
